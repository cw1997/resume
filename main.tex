...
\begin{document}
  ...
  \begin{itemize}
    \item \textbf{前端}: JavaScript/TypeScript, React.js(Next.js, Gatsby,js), HTML/CSS(Sass, TailwindCSS，styled-component), Vue.js(Nuxt), jQuery, Webpack, Gulp.js, Rollup, Vite, Turborepo
  ...
  \datedsubsection
  {\textbf{Mininggoat（北京明光振铎数据科技有限公司）}| 前端开发与系统运维}
  {远程 | 2026/01 \textasciitilde 2026/09}
  \begin{itemize}
    \item 主要负责挖数羊智能数据交易平台（washuyang.com）的前端开发与系统运维工作，包括平台前端功能开发、页面优化、系统维护及相关技术支持等
    \item 使用 React、Next.js等技术栈、结合Cursor Gork，OpenCode(DeepSeek)等AI Coding工具加速开发
  \end{itemize}

  \datedsubsection
  {\textbf{RisingWave Labs（北京奇点无限数据科技有限公司）}| 前端开发}
  {远程 | 2024/05 \textasciitilde 2025/02}
  \begin{itemize}
  ...
  \datedsubsection
  {\textbf{PingCAP（北京平凯星辰科技发展有限公司）}| 前端开发}
  {北京，深圳，远程 | 2020/10 \textasciitilde 2024/04}
  \begin{itemize}
  ...
  \iconsection{graduation-cap}{教育经历}

  \datedsubsection
  {\textbf{台湾师范大学（NTNU）}| 社会教育学系 | \textit{教育学博士（在读）}}
  {2025/09 \textasciitilde 至今}
  \begin{itemize}
    \item 担任RA(Research Assistant)，协助教授进行实验进行AI+教育相关研究，使用Dify + Next.js技术栈
  \end{itemize}

  \datedsubsection
  {\textbf{台湾科技大学（NTUST）}| 电子工程系 | \textit{工学硕士}}
  {2021/09 \textasciitilde 2025/08}
  \begin{itemize}
    \item \textbf{光电及半导体组}。研究基于PCB电路板、半导体元件和各种新型材料设计毫米波空间滤波器/天线。获校级奖学金
  \end{itemize}

  \datedsubsection
  {\textbf{台湾科技大学（NTNST）}| 电子工程系 | \textit{工学学士}}
  {2018/09 \textasciitilde 2021/08}
  \begin{itemize}
    \item \textbf{毕业设计:使用VerilogHDL在FPGA上构建RISC-V指令集架构的CPU}(\textit{https://github.com/risc-v-cpu})。
    熟悉计算机体系结构，操作系统，计算机网络，算法与数据结构，数据库，电子电路，MCU单片机(STM32, Arduino, 51单片机)，FPGA(SystemVerilogHDL)，PCB电路板设计，SMT贴片/THT插件元件焊接。获书卷奖（班级排名第一）
  \end{itemize}

  \datedsubsection
  {\textbf{武汉船舶职业技术学院}| 软件技术专业 | \textit{专科/高职}}
  {2015/09 \textasciitilde 2018/08}
  \begin{itemize}
    \item \textbf{主要学习Java/PHP等Web前后端开发相关技术}。担任班级学习委员，负责班级助教工作。获校级奖学金
  \end{itemize}

  \iconsection{list}{项目作品}

  \datedsubsection
  {\textbf{NTUST LIB}\textit{（手机APP）}}
  {https://library.ntust.edu.tw/p/405-1049-110462,c11171.php?Lang=zh-tw}
  \begin{itemize}
    \item \textbf{前端}基於React Native with Expo SDK開發，使用TypeScript程式語言。
    支援跨平台使用『iOS、Android 作業系統』，并且同时上架AppStore和GooglePlay，接入Firebase进行埋点分析，crash监控，接入Sentry收集日志
  ...
  \datedsubsection
  {\textbf{ez-react}\textit{（React.js复刻版）}}
  ...
  \datedsubsection
  {\textbf{SDRAM Controller, SDRAM内存控制器}}
  ...
  \datedsubsection
  {\textbf{Google Developer Group on Campus}（谷歌开发者校园社群）| 讲师}
  {}
  \begin{itemize}
    \item \textbf{台湾师范大学}校区讲师，负责为社团成员\textbf{讲解React和Next.js前端开发相关技术和SQL/Postgres/Supabase数据库相关技术}
  ...
  {}
  \begin{itemize}
    \item \textbf{行政部门}财务部部长，\textbf{负责会计工作，编列预决算报表}。
    学权部部员，\textbf{协助学校分发意见调查表单、举办校园演唱会}。
    \item \textbf{立法部门(学生议会)}副议长，\textbf{负责协助议会开会事项，审理学生会行政部门的预决算报表，审核活动办理情况}
  \end{itemize}

  \datedsubsection
  {\textbf{武汉船舶职业技术学院 新媒体中心} | 部员}
  {}
  \begin{itemize}
